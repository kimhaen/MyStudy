...
> 나는 자바를 이클립스와 함께 시작했기 때문에, 현재는 프로젝트 생성 및 컴파일, 빌드, 실행의 전반적인 절차가 자동으로 이루어지는 환경에 익숙하다. 그러나 이런 IDE 가 존재하지 않았던 자바의 초창기 시절도 있었을 것이고, 선대 개발자들은 그러한 환경에서도 얼마든지 코딩을 해왔을 것이다.
...
1. [자바 기본 개념 정리](#자바-기본-개념-정리)
2. [자바 프로젝트 생성 및 실행](#자바-프로젝트-생성-및-실행)
3. [느낀점](#느낀점)
...
```
GC 는 클래스 로더와 같은 수준에서 힙 영역에 대해 동작하며,
네이티브 메서드 영역은 자바 이외의 언어로 작성된 네이티브 코드를 Java Native Interface(JNI) 로 호출해 적재하는 공간이다.
...
4. **컴파일 및 실행**
	* 자바 관련 실행 프로그램은 시스템에 지정한 Java 경로 내 bin (바이너리, 실행 파일들의 모음) 디렉토리에 위치한다
...
- 구성
	* javac [옵션] [목적 자바 소스파일]
- 옵션
	* -d : 컴파일된 클래스파일이 생성되는 경로를 지정. 이 옵션을 지정하지 않으면 소스코드와 같은 디렉토리에 생성된다
...
- 추후 유사시 자바만 설치되어 있다면 날코딩으로도 작업을 할 수 있겠다는 생각을 함 (규모가 큰 프로젝트에서는 말도 안되는 생각일거같긴 함)
